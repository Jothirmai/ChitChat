...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (userExists) {
    return res.status(400).json({
      message: "Your E-Mail Id is already Registered with Chit-Chat",
      success: false,
    });
  }
  if (contactExists) {
    return res.status(400).json({
      message: "Your Mobile is already Registered with Chit-Chat",
      success: false,
    });
  }
  ...
  if (user) {
    ...
    const options = {
      ...
      message_Content:
        "<p> Hi " +
        user.name +
        ",<br /> Please verify your Chit-Chat Account by clicking on the verification link. This Verification link is valid for 2:00 minutes <br /> <a href =" +
        url +
        " >Verify</a></p> ",
    };
